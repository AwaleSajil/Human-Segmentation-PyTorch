...
###match label and img pair in sequence

for Lpath, Dpath in zip(data_labels, datas):
  #first correct the complete path
  LpathCom = completePath(Lpath)
  DpathCom = completePath(Dpath)

  #find all the imagespath in labels and data
  label_paths = sorted(glob(LpathCom), key = numericalConvert)
  data_paths = sorted(glob(DpathCom), key = numericalConvert)

  label_num = [numericalConvert(p) for p in label_paths]
  data_num = [numericalConvert(p) for p in data_paths]

  for idx, label_path in enumerate(label_paths):
    if label_num[idx] in data_num:
      #if labeled image is in data
